...
heatmap2_no_en = heatmap_no_en.T # switch rows and columns
plt.figure(figsize=(12, 10))
sns.heatmap(heatmap2_no_en, cmap='Reds', annot=True, linewidths=0.5)
plt.title("Heatmap of Language Usage Excluding English (2018-2022)")
...
# wordcloud
def clouding(data, mask = None, save_path = None):
  ...
  plt.axis('off')
  plt.title(f"Word Cloud of Language Trends for {country_name} (2018-2022)")
  if save_path:
...
for country in countries:
    ...
    if country_name != "Global": # use masks for countries
        clouding(language_counts, mask=masks[country_name],
                 save_path=f'/Users/tian_mac/sfu/lab1/project/plots/{country_name}_wordcloud.png')
    else:
        clouding(language_counts, None,
                 save_path=f'/Users/tian_mac/sfu/lab1/project/plots/{country_name}_wordcloud.png')
